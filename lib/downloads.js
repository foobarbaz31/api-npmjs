...
function fetchTotalDownloads(opts, callback) {
  ...
  async.parallel(jobs, (err, result) => {
    ...
    const itemsNoError = lodash.filter(flattenedResult, item =>
       !(item.error && lodash.includes(item.error, 'no stats for this package for this period'))
    );

    const checkError = lodash.find(itemsNoError, 'error');
    if (checkError) {
      return callback(new Error(checkError.error));
    }

    const downloads = lodash.map(itemsNoError, 'downloads');
...
function fetchRangeDownloads(opts, callback) {
  ...
  async.parallel(jobs, (err, result) => {
    ...
    const itemsNoError = lodash.filter(flattenedResult, item =>
       !(item.error && lodash.includes(item.error, 'no stats for this package for this range'))
    );
    ...
    const downloads = lodash.flatten(lodash.map(itemsNoError, 'downloads'));
